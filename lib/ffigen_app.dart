...
final DynamicLibrary _dylib = (){
  if(Platform.isMacOS || Platform.isIOS){
    if(Platform.environment.containsKey('FLUTTER_TEST')){
      return DynamicLibrary.open('build/macos/Build/Product/Debug'
      '/$_libName/$_libName.framework/$_libName');
    }
    return DynamicLibrary.open('$_libName.framework/$_libName');
  }
  if(Platform.isAndroid||Platform.isLinux){
    if(Platform.environment.containsKey('FLUTTER_KEY')){
...
class Duktape {
  ...
  String evalString(String jsCode){
    ...
    final evalResult = _bindings.duk_eval_raw(ctx, nativeUtf8.cast<Char>(), 0, 0 | 
        DUK_COMPILE_EVAL |
        DUK_COMPILE_SAFE |
  ...
  String _retrieveTopOfStackAsString(){
    ...
    final returnValue = errorStrPtr.cast<ffi.Utf8>().toDartString(length: outLengthPtr.value);
